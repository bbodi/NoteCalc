...
class LineParser {
    ...
    private fun createHighlightingNamesForTokens(tokens: List<Token>): List<NoteCalcEditor.Companion.HighlightedText> {
        val highlightInfosForTokens = arrayListOf<NoteCalcEditor.Companion.HighlightedText>()
        tokens.forEach { token ->
            when (token) {
                is Token.NumberLiteral -> {
                    ...
                    highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(strRepr, "number"))
                }
                is Token.Variable -> {
                    // TODO jelenleg ez a syntaxhilighterben van szinezve, nem lenne jobb itt? iugy h eltárolom a változó nevét is
                    highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(token.variableName, "variable"))
                }
                is Token.StringLiteral -> {
                    highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(token.str, "comment"))
                }
                is Token.Operator -> {
                    highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(token.operator, "operator"))
                }
                is Token.UnitOfMeasure -> {
                    if (token.tokens.isEmpty()) {
                        highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(token.unitName, "qualifier"))
                    } else {
                        token.tokens.forEach { subTokenInCompoundUnit ->
                            highlightInfosForTokens.add(NoteCalcEditor.Companion.HighlightedText(getStringRepresentation(subTokenInCompoundUnit), "qualifier"))
                        }
                    }
                }
            }
    ...
    internal data class EvaulationResult(
            ...
            val highlightedTexts: List<NoteCalcEditor.Companion.HighlightedText>,
            val lastToken: Token?
    )
